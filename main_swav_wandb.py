...
wandb.init(project="Swav-12", entity="marlhakizi")
...
from src.RGBmulticrop import MultiCropDataset
from src.Twelve_multicrop import TwelveMultiCropDataset
...
parser.add_argument(
    ...
    default=3,
    type=int,
    help="The number of channels needs for inputs",
)
parser.add_argument(
    "--nmb_crops",
    type=int,
...
def main():
    ...
    logger, training_stats = initialize_exp(args, "epoch", "loss", "outputs")
    wandb.config = {
        "epochs": args.epochs,
        "batch_size": args.batch_size,
        "Number of channels": args.number_channels,
    }
    # build data
    if args.number_channels == 12:
        train_dataset = TwelveMultiCropDataset(
            args.task,
            args.data_path,
            args.size_crops,
            args.nmb_crops,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        new_out = y_outputs  # [i.cpu().detach().numpy()for i in embes]
        with open("parrot.pkl", "wb") as f:

            pickle.dump(new_out, f)
        training_stats.update(scores)
        all_outputs.append(new_out)
        # save checkpoints
        if args.rank == 0:
            save_dict = {
                "epoch": epoch + 1,
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue):
    ...
    for it, inputs in enumerate(train_loader):
        ...
        embedding, output, y_output = model(inputs)
    ...
    # if isinstance(losses.avg, torch.Tensor):
    # ls = losses.avg.cpu().numpy()
    dict_losses = json.loads(json.dumps(losses.__dict__))

    wandb.log(
        {
            "Epoch": epoch,
            "Loss": dict_losses["avg"],
            "Lr": optimizer.optim.param_groups[0]["lr"],
        }
    )
    # return (epoch, losses.avg), queue, y_outputs
    # return (epoch, losses.avg), queue
    return (epoch, losses.avg, y_outputs), queue, y_outputs
    # return (epoch, losses.avg,y_outputs), queue,y_outputs
