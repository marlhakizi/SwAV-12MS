...
# Replace "service_account" with own service account address
# Replace "json_key" with own JSON file path
#############################################################
...
def time_generator():
    """
    Pick a start date and end date for each location
    Edit "date_range" as needed, can comment out random.choice if needed
    """
    date_range = [
...
def generateURL(
    ...
    if dataset == "sentinel":
        # filter out images with greater than 10% cloud coverage
        cloud_pct = 10
...
##########################################################
# Replace "output_dir" with own output directory file path
# Replace "filepath" with own coordinates file path
##########################################################
if __name__ == "__main__":
    args = {
        "dataset": "sentinel",
        "output_dir": "/Users/marlynehakizimana/Documents/FALL22/climate-eye/f/batch/",
        "filepath": "Data/coordinates.csv",
    ...
    for i in tqdm(range(0, len(data), 1000)):
        ...
        logging.info(f"Starting rows: {i} to {i+1000}")
        # export_start_time = time.time()
        # print(f"Starting rows: {i} to {i+pn}")
        # logging.info(f"Starting rows: {i} to {i+pn}")
        start = time.time()
        pool.map(lat_lon_only, data[i : i + 1000])
        time.sleep(1)
