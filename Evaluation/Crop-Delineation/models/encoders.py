...
def load(encoder_name):
    if encoder_name == "swav":
        ...
        return _load_swav_pretrained(
            "/home/mh613/updatedswav/rgbpaaath/ckp-48-eval.pth"
        )
    elif encoder_name == "swav-12":
        print("Loading 12 channel swav pretrained weights.")
        return _load_12swav_pretrained(
            "/home/mh613/updatedswav/millionpaath/checkpoints/ckp-eval.pth"
        )
...
def _load_12swav_pretrained(model_path):
    """
    This function loads the swav encoder pretrained on the solar training
    dataset.
    """
    state_dict = torch.load(model_path)
    base_model = resnet.resnet50(inter_features=True)
    base_model.conv1 = nn.Conv2d(12, 64, kernel_size=7, stride=2, padding=3, bias=False)
    base_model.load_state_dict(state_dict)
...
def _load_imagenet():
    ...
    """
    model = r50(weights=ResNet50_Weights.IMAGENET1K_V2)
